...
SKILLS: official herdr (npx skills add herdrdev/herdr --skill herdr -g)
        <kit>/skills/agent-behavior/SKILL.md sha256=<…>   # Herdr extras + pane onboard
DO: Full-read both. ACK MATCH. Role = your tab + roster (not vendor). Idle until a work packet.
CALLBACK: herdr agent prompt <requester_pane>  (EMAIL, once)
```

After the lead ACKs, send them one more EMAIL: onboard every other pane with **herdr + agent-behavior** the same way; harvest ACKs; then idle.
...
AGENT_BEHAVIOR_SHA256: <hash> MATCH
NEXT: idle
```
